...
public class HealthPlanModule {  
    ...
    // 评估计划效果并提供反馈  
    public void evaluatePlan(int planId) throws SQLException {  
        PreparedStatement pstmt = connection.prepareStatement("SELECT status FROM ExecutionTracking WHERE planId = ?");  
        pstmt.setInt(1, planId);  
        ResultSet rs = pstmt.executeQuery();  
        ...
        while (rs.next()) {  
            totalDays++;  
            if ("完成".equals(rs.getString("status"))) {  
                completedDays++;  
            }  
        }  
        
        if (totalDays == 0) {  
            System.out.println("没有执行记录。请记录后再进行评估。");  
        } else {  
            double completionRate = (double) completedDays / totalDays * 100;  
            System.out.printf("完成率: %.2f%%\n", completionRate);  
            provideFeedback(completionRate);  
        }  
    ...
    // 提供反馈建议  
    private void provideFeedback(double completionRate) {  
        if (completionRate == 100) {  
            System.out.println("优秀！您完全按照计划执行，继续保持！");  
        } else if (completionRate >= 75) {  
            System.out.println("很好！您基本完成了计划，继续努力！");  
        } else if (completionRate >= 50) {  
            System.out.println("尚可。尽量提高执行率，加油！");  
        } else {  
            System.out.println("需要更多努力。建议重新审视计划，设定更可实现的目标。");  
        }  
    }  
    ...
    public static void main(String[] args) {  
        try {  
            ...
            while (true) {  
                System.out.println("请选择操作: 1. 制定健康计划 2. 查询健康计划 3. 记录计划执行情况 4. 查询执行记录 5. 评估计划效果 6. 退出");  
                ...
                if (choice == 1) {  
                    ...
                    System.out.print("请输入健康计划的 ID 进行评估: ");  
                    int planId = scanner.nextInt();  
                    hpm.evaluatePlan(planId);  
                } else if (choice == 6) {  
